...
module lab_6_top_level (
    ...
    logic [6:0]  daddr_in;           // XADC address (not really used here)
    logic        enable;             // XADC enable (using eoc_out)
    ...
    logic waveform_select;  // 0 = triangle, 1 = sawtooth
    ...
    // LEDs: pulse with PWM, show scaled ADC
    // (we'll replace pwm_out_internal later with selected waveform)
    // for now just declare the signal
    logic wave_pwm_for_led;
    assign led = wave_pwm_for_led ? scaled_adc_data : '0;
    ...
    // NOTE: ave_data was in your original mux; if it's defined elsewhere, keep it.
    // If not, comment that input out or hook it up properly.
    logic [15:0] ave_data;  // declare to avoid compile errs; connect if you have it
    ...
    mux4_16_bits MUX4 (
        ...
        .in3   (ave_data),          // averaged before scaling
        .select(bin_bcd_select),
        .mux_out(mux_out),
        .decimal_point(decimal_pt)
    );

    logic [15:0] mux_out_pulsed;
    
    assign mux_out_pulsed = wave_pwm_for_led ? mux_out : '0;

    // Seven Segment Display Subsystem
    seven_segment_display_subsystem SEVEN_SEGMENT_DISPLAY (
        .clk(clk),
        .reset(reset),
        .sec_dig1(mux_out_pulsed[3:0]),
        .sec_dig2(mux_out_pulsed[7:4]),
        .min_dig1(mux_out_pulsed[11:8]),
        .min_dig2(mux_out_pulsed[15:12]),
        .decimal_point(decimal_pt),
    ...
    // turn them on when in PWM or R2R mode
    logic wave_en;
    assign wave_en = pwm_enable | r2r_enable;

    // triangle signals
    logic        tri_pwm_out;
    logic [7:0]  tri_r2r_out;
    ...
    triangle_generator #(
        ...
        .clk    (clk),
        .reset  (reset),
        .enable (wave_en & (waveform_select == 1'b0)),  // FSM says triangle
        .pwm_out(tri_pwm_out),
        .R2R_out(tri_r2r_out)
    );
    ...
    sawtooth #(
        ...
        .clk    (clk),
        .reset  (reset),
        .enable (wave_en & (waveform_select == 1'b1)),  // FSM says sawtooth
        .pwm_out(saw_pwm_out),
        .R2R_out(saw_r2r_out)
    );
    ...
    // pick final waveform
    logic        wave_pwm_out;
    logic [7:0]  wave_r2r_out;

    always_comb begin
        if (waveform_select) begin
            wave_pwm_out = saw_pwm_out;
            wave_r2r_out = saw_r2r_out;
        end else begin
            wave_pwm_out = tri_pwm_out;
            wave_r2r_out = tri_r2r_out;
        end
    end

    // we also use this for LED pulsing
    assign wave_pwm_for_led = wave_pwm_out;

    logic buzzer_fixed_out;
    logic buzzer_chirp_out;
   
    // old fixed 1 kHz buzzer (optional, keep)
    buzzer_pwm #(
        .CLOCK_FREQ(100_000_000),
        .BUZZER_FREQ(1000)
    ) buzzer_fixed_inst (
        .clk    (clk),
        .reset  (reset),
        .enable (1'b0),                 // <-- keep off for now, or add another mode later
        .pwm_out(buzzer_fixed_out)
    );
    ...
    buzzer_chirp #(
        ...
        .FREQ_MAX  (10_000),
        .FREQ_STEP (100),
        .STEP_TICKS(5_000_000)
    ) buzzer_chirp_inst (
        .clk    (clk),
        .reset  (reset),
        .enable (buzzer_enable),        // <-- from FSM state 11
        .pwm_out(buzzer_chirp_out)
    );

    always_comb begin
        pwm_out    = pwm_enable ? wave_pwm_out   : 1'b0;
        R2R_out    = r2r_enable ? wave_r2r_out   : '0;
        buzzer_out = buzzer_enable ? buzzer_chirp_out : 1'b0;
...
//     // Enable when in PWM or R2R mode
//    logic wave_en;
//    assign wave_en = pwm_enable | r2r_enable;
...
//    always_comb begin
//        pwm_out    = pwm_enable ? saw_pwm_out : 1'b0;
//        R2R_out    = r2r_enable ? saw_r2r_out : '0;
//        buzzer_out = buzzer_enable ? buzzer_out_internal : 1'b0;
//    end
